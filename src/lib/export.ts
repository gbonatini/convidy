...
interface CheckInRegistration {
  ...
  events?: { // For CheckIn page compatibility
    title: string;
    date: string;
    location: string;
  };
...
export const exportConfirmations = (registrations: Registration[], eventFilter?: string) => {
  const data = registrations.map(reg => ({
    ...
    'Email': reg.email,
    'Telefone': reg.phone || '-',
    'CPF': reg.document?.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4') || '-',
    'Status': reg.status === 'confirmed' ? 'Confirmado' : 'Cancelado',
    'Check-in': reg.checked_in ? 'Sim' : 'Não',
    'Data Check-in': reg.checkin_time ? format(new Date(reg.checkin_time), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR }) : '-',
...
export const exportInvites = (invites: Invite[], eventFilter?: string) => {
  const data = invites.map(invite => ({
    ...
    'WhatsApp': invite.whatsapp?.replace(/(\d{2})(\d{5})(\d{4})/, '($1) $2-$3') || '-',
    'Email': invite.email || '-',
    'Status': getInviteStatusText(invite.status),
    'Evento': invite.events.title,
    'Data do Evento': format(new Date(invite.events.date + 'T00:00:00'), "dd/MM/yyyy", { locale: ptBR }),
...
export const exportCheckIns = (registrations: any[], eventFilter?: string) => {
  ...
  const checkedInData = registrations.filter(reg => reg.checked_in);
